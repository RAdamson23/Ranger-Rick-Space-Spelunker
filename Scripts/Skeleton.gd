...
var ANIMATIONS = {"IDLE": "Idle","RUN": "Run","ATTACK": "Attack","HIT": "Hit","DEAD": "Dead","BLOCK":"Block"}
...
func switchPos():
	...
	$ShieldHitbox/CollisionShape2D.position.x *= -1
	$FireballDetector/CollisionShape2D.position.x *= -1
func _process(delta):
...
func onHit():
	if !shieldHit:
		if !hasSeenPlayer:
			hasSeenPlayer = true
			firstrun = false
		$FireballDetector/CollisionShape2D.call_deferred("set_disabled", true)
		isHit = true
		changeAnimation("HIT")
		$Hit.play()
		health-=1
		...
		$FireballDetector/CollisionShape2D.call_deferred("set_disabled", false)
		isHit = false
	else:
		shieldHit = false
func dead():
	...
	$FireballDetector/CollisionShape2D.call_deferred("set_disabled", true)
	$ShieldHitbox/CollisionShape2D.call_deferred("set_disabled", true)
	velocity.x = 0
	while !is_on_floor():
		velocity.y+=GRAVITY
...
func block():
	if !isblocking:
		...
		isblocking = true
		$ShieldHitbox/CollisionShape2D.call_deferred("set_disabled", false)
		velocity.x = 0
		velocity.y += GRAVITY
		velocity = move_and_slide(velocity,FLOOR)
...
func _on_FireballDetector_body_entered(body):
	
		if(body.is_in_group("Fireball")):
			if $BlockCooldown.is_stopped():
				block()
